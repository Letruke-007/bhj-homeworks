...
cart.addEventListener('click', function(event) {
    ...
    if (target.classList.contains('cart__product-remove')) {
        let findSame = cartProducts.findIndex(el => el.id === target.closest('.cart__product').dataset.id);
...
products.addEventListener('click', function(event) {
    ...
    if (target.classList.contains('product__quantity-control_dec')) {
        changeQuantity = target.closest('.product__quantity-controls').querySelector('.product__quantity-value');
    ...
    if (target.classList.contains('product__add')) {
        ...
        let productImg = target.closest('.product').querySelector('.product__image');
        let productCount = parseInt(target.closest('.product__quantity').querySelector('.product__quantity-value').innerText);
        let obj;
        ...
        renderProducts(cartProducts);
        let findSame = arrayForGetCoords.findIndex(el => el.dataset.id === productId);
        if (findSame !== (-1)) {
            coordsImgCartProduct = arrayForGetCoords[findSame].querySelector('.cart__product-image').getBoundingClientRect();
            getCoords(findSame, coordsImgProduct);
        }
        toLocalStorage = JSON.stringify(cartProducts);
        localStorage.setItem('cart', toLocalStorage);
...
function renderProducts(array) {
    ...
    arrayForGetCoords = Array.from(cart.querySelector('.cart__products').querySelectorAll('.cart__product'));
...
function getCoords(index, coordsImgProduct) {
    coordsImgCartProduct = arrayForGetCoords[index].querySelector('.cart__product-image').getBoundingClientRect();
    let imgClone = arrayForGetCoords[index].querySelector('.cart__product-image').cloneNode(true);
